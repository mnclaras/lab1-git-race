...
if [ "$TRAVIS_REPO_SLUG" == "mnclaras/lab1-git-race" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ]; then
  if [[ $(./gradlew -q getVersion) != *SNAPSHOT* ]]; then
      echo 'Travis can only publish snapshots. To publish a release, use the mnclaras Jenkins instance.'
      return 0
  fi
