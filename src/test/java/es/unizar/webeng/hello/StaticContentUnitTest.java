...
*   {@code @RunWith(SpringRunner.class)} Annotation from Junit, Junit will run using Spring's testing support.
*    JUnit will invoke the SpringRunner class to run the tests in that class instead of the runner built into JUnit.
*    {@code SpringRunner} is the new name for {@code SpringJUnit4ClassRunner} <br>
*
*   {@code @WebMvcTest(HelloController.class)} Start the Spring application context without the server, 
*   in that case Spring Boot is only instantiating one controller {@code HelloController}.
*   Using this annotation will disable full auto-configuration and instead apply only configuration relevant to MVC tests.<br>
*
*   {@code @Value("$app.message:Hello World")} Annotation that indicates a default value expression for the affected argument.
*   We can inject the value from a file into a variable with that syntax {@code  @Value("$value.from.file")}, in case it's not defined 
*   we use double dots {@code @Value("$unknown.param:Hello World")} and the text Hello World will be injected.<br>
*
*   {@code @Autowired} Annotation to auto wire bean on the field. This annotation will scan on the packpage for annotations like Controller, 
